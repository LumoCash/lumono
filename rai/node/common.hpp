...
public:
	...
	static std::array<uint8_t, 2> constexpr magic_number = rai::rai_network == rai::rai_networks::rai_test_network ? std::array<uint8_t, 2>{ { 'L', 'O' } } : rai::rai_network == rai::rai_networks::rai_beta_network ? std::array<uint8_t, 2>{ { 'L', 'N' } } : std::array<uint8_t, 2>{ { 'L', 'M' } };
	uint8_t version_max;
